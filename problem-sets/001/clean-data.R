...
# Setup ----------------------------------------------------------------------------------
  # Packages
  library(pacman)
  p_load(data.table, collapse, stringr, fixest, here)

# Load dataset ---------------------------------------------------------------------------
  # Load the raw data
  acs_dt = here('problem-sets/001/data-raw/usa_00007.csv') |> fread()

# Clean data -----------------------------------------------------------------------------
  # Names to lowercase
  setnames(acs_dt, tolower(names(acs_dt)))
  # Pad FIPS codes
  acs_dt[, statefip := str_pad(statefip, 2, pad = '0')]
  acs_dt[, countyfip := paste0(statefip,  str_pad(countyfip, 3, pad = '0'))]
  setnames(acs_dt, c('statefip', 'countyfip'), c('state', 'county'))
  # Recode sex
  acs_dt[, `:=`(
    sex = fcase(
      sex == 1, 'Male',
...
# Save data ------------------------------------------------------------------------------
  ...
  fwrite(
    x = sub_dt,
    file = here('problem-sets', '001', 'data-acs.csv')
  )
